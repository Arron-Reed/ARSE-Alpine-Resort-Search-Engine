...
router.get("/:id", VerifyTokenMiddleware_1.default, async (req, res) => {
    const userId = req.params.id;
    try {
        const getUser = await userModel_1.UserModel.findOne({ _id: userId });
        if (!getUser) {
...
router.post("/register", async (req, res) => {
    try {
        ...
        ;
        const userAvailable = await userModel_1.UserModel.findOne({ email });
        if (userAvailable) {
            res.status(400);
            throw new Error("User already registered");
        }
        ;
        const hashedPassword = await bcrypt_1.default.hash(password, 1);
...
router.post('/savefavourite', VerifyTokenMiddleware_1.default, async (req, res) => {
...
router.get("/favourites/:userId", VerifyTokenMiddleware_1.default, async (req, res) => {
    ...
    try {
        ...
        const favoriteSkifields = await skifieldModel_1.SkifieldModel.find({ _id: { $in: favourites } });
...
router.delete("/favourites/:userId/:skifieldId", VerifyTokenMiddleware_1.default, async (req, res) => {
    ...
    try {
        ...
        const updatedUser = await userModel_1.UserModel.findByIdAndUpdate(userId, { $pull: { favourites: skifieldId } }, { new: true });
...
router.post('/beenhere', VerifyTokenMiddleware_1.default, async (req, res) => {
    const _id = req.body.userId;
    const skifieldId = req.body.skifieldId;
    try {
        const user = await userModel_1.UserModel.findById(_id);
...
router.get("/beenhere/:userId", VerifyTokenMiddleware_1.default, async (req, res) => {
    const userId = req.params.userId;
    try {
        const user = await userModel_1.UserModel.findById(userId);
        if (!user) {
        ...
        const beenhereSkifields = await skifieldModel_1.SkifieldModel.find({ _id: { $in: beenhere } });
...
router.delete("/beenhere/:userId/:skifieldId", VerifyTokenMiddleware_1.default, async (req, res) => {
    const { userId, skifieldId } = req.params;
    try {
        const user = await userModel_1.UserModel.findById(userId);
        if (!user) {
        ...
        const updatedUser = await userModel_1.UserModel.findByIdAndUpdate(userId, { $pull: { beenHere: skifieldId } }, { new: true });
